...
COMMANDS
  ...
  logging s3 create --name=NAME --version=VERSION --bucket=BUCKET [<flags>]
    ...
    -s, --service-id=SERVICE-ID  Service ID (falls back to FASTLY_SERVICE_ID,
        ...
        --timestamp-format=TIMESTAMP-FORMAT
                                 strftime specified timestamp formatting
                                 (default "%Y-%m-%dT%H:%M:%S.000")
        --redundancy=REDUNDANCY  The S3 storage class. One of: standard,
                                 intelligent_tiering, standard_ia, onezone_ia,
                                 glacier, glacier_ir, deep_archive, or
                                 reduced_redundancy
        --placement=PLACEMENT    Where in the generated VCL the logging call
                                 should be placed, overriding any format_version
                                 default. Can be none or waf_debug
  ...
  tls-custom certificate create --cert-blob=CERT-BLOB [<flags>]
    ...
    --name=NAME            A customizable name for your certificate. Defaults to
                           the certificate's Common Name or first Subject
  ...
  tls-subscription update --id=ID [<flags>]
    Change the TLS domains or common name associated with this subscription, or
    update the TLS configuration for this set of domains
