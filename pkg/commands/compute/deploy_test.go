...
func TestDeploy(t *testing.T) {
	...
	scenarios := []struct {
		...
		// The first is if we need to bypass the entitlement API call.
	...
	}{
		{
			...
			wantRemediationError: fsterr.ComputeInitRemediation,
			noManifest:           true,
		},
		{
			...
			name: "success with no service ID",
		...
		{
			// This test validates a new service creation for someone either without
			// paid access to the Compute product, or who are already using the trial
			// and so they don't need to be reminded each time that their service is
			// part of a trial.
			name: "success with no service ID and showing trial message",
			args: args("compute deploy --token 123 -v --package pkg/package.tar.gz"),
			api: mock.API{
				ActivateVersionFn: activateVersionOk,
				CreateBackendFn:   createBackendOK,
		...
		},
		{
			...
			wantError: "error checking entitlement to the Compute product: whoops: 400 Bad Request",
		},
		...
		},
		{
			...
			wantRemediationError: fsterr.PackageSizeRemediation,
		},
	...
	for testcaseIdx := range scenarios {
		...
		t.Run(testcase.name, func(t *testing.T) {
			...
			if testcase.httpClientRes != nil || testcase.httpClientErr != nil {
				...
				// Default to mocking Compute entitlement check to be successful.
				opts.HTTPClient = mock.HTMLClient(
					[]*http.Response{
						{
							Body:       io.NopCloser(strings.NewReader(`{"has_access":true}`)),
							Status:     http.StatusText(http.StatusOK),
							StatusCode: http.StatusOK,
						},
					},
