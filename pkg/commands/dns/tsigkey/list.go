...
// ListCommand calls the Fastly API to list TSIG keys.
...
func NewListCommand(parent argparser.Registerer, g *global.Data) *ListCommand {
	...
	c.CmdClause = parent.Command("list", "List TSIG keys")

	// Optional.
	c.CmdClause.Flag("name", "Filter TSIG keys to only those containing the provided name.").Action(c.name.Set).StringVar(&c.name.Value)
...
func (c *ListCommand) Exec(_ io.Reader, out io.Writer) error {
	...
	keys, err := tsigkeys.List(context.TODO(), fc, input)
	if err != nil {
