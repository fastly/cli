...
// CreateCommand calls the Fastly API to create a Mailing List notification integration.
...
func NewCreateCommand(parent argparser.Registerer, g *global.Data) *CreateCommand {
	...
	c.CmdClause = parent.Command("create", "Create a Mailing List notification integration").Alias("add")

	// Required.
	c.CmdClause.Flag("name", "The name of the integration").Short('n').Required().StringVar(&c.IntegrationName)
	c.CmdClause.Flag("address", "The mailing list address to notify").Required().StringVar(&c.Address)
...
func (c *CreateCommand) Exec(_ io.Reader, out io.Writer) error {
	...
	input := &fastly.CreateIntegrationInput{
		...
		Config: map[string]string{"address": c.Address},
	}
	...
	text.Success(out, "Created Mailing List integration '%s' (id: %s)", c.IntegrationName, fastly.ToValue(o.ID))
	return nil
}
