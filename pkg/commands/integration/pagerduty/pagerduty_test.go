...
import (
	...
	sub "github.com/fastly/cli/pkg/commands/integration/pagerduty"
...
func TestCreateCommand(t *testing.T) {
	const (
		...
		integrationKey  = "a1b2c3d4"
	)

	scenarios := []testutil.CLIScenario{
		{
			Args:      fmt.Sprintf("--key %s", integrationKey),
		...
		{
			Args: fmt.Sprintf("--name %s --key %s", integrationName, integrationKey),
			API: &mock.API{
		...
		},
		{
			...
			API: &mock.API{
				CreateIntegrationFn: func(_ context.Context, i *fastly.CreateIntegrationInput) (*fastly.CreateIntegrationResponse, error) {
					...
					if i.Config["key"] != integrationKey {
						return nil, fmt.Errorf("unexpected key: %s", i.Config["key"])
					}
					return &fastly.CreateIntegrationResponse{ID: fastly.ToPointer(integrationID)}, nil
				},
			},
			WantOutput: fstfmt.Success("Created PagerDuty integration '%s' (id: %s)", integrationName, integrationID),
		},
		{
			Args: fmt.Sprintf("--name %s --key %s --json", integrationName, integrationKey),
			API: &mock.API{
				CreateIntegrationFn: func(_ context.Context, _ *fastly.CreateIntegrationInput) (*fastly.CreateIntegrationResponse, error) {
					return &fastly.CreateIntegrationResponse{ID: fastly.ToPointer(integrationID)}, nil
				},
...
func TestUpdateCommand(t *testing.T) {
	...
	scenarios := []testutil.CLIScenario{
		...
		{
			Args: fmt.Sprintf("%s --key %s", integrationID, integrationKey),
			API: &mock.API{
		...
		},
		{
			...
			API: &mock.API{
				UpdateIntegrationFn: func(_ context.Context, i *fastly.UpdateIntegrationInput) error {
					...
					if i.Config["key"] != integrationKey {
						return fmt.Errorf("unexpected key: %s", i.Config["key"])
					}
					return nil
				},
			},
			WantOutput: fstfmt.Success("Updated PagerDuty integration (id: %s)", integrationID),
		},
		{
			Args: fmt.Sprintf("%s --key %s --json", integrationID, integrationKey),
			API: &mock.API{
				UpdateIntegrationFn: func(_ context.Context, _ *fastly.UpdateIntegrationInput) error {
					return nil
				},
