...
func (c *ListCommand) Exec(in io.Reader, out io.Writer) error {
	...
	rs, err := c.Globals.Client.ListUsers(input)
	if err != nil {
		c.Globals.ErrLog.AddWithContext(err, map[string]interface{}{
			"Service ID": serviceID,
...
func (c *ListCommand) constructInput(serviceID string) *fastly.ListUsersInput {
	var input fastly.ListUsersInput
...
// format.
func (c *ListCommand) printVerbose(out io.Writer, serviceID string, serviceVersion int, rs []*fastly.User) {
	fmt.Fprintf(out, "\nService ID: %s\n", serviceID)
