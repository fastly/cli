...
func NewDeactivateCommand(parent common.Registerer, globals *config.Data) *DeactivateCommand {
	...
	c.CmdClause.Flag("version", "Number of version you wish to deactivate").Required().IntVar(&c.Input.Version)
	return &c
}
...
func (c *DeactivateCommand) Exec(in io.Reader, out io.Writer) error {
	...
	text.Success(out, "Deactivated service %s version %d", v.ServiceID, c.Input.Version)
	return nil
}
